    ...
    <main>
        ...
        <div class="main_page_btn">
        	...
        	if(currentPage % grpSize ==0){
			 ...
			 if(currentGrp >1){
			%>
			<a href="getpagebooks?page=<%= index-1 %>&code=<%=code%>&st=<%= st%>">[이전]</a>
			<%
					}	 
			 while( index <= grpEndPage){
			 
			 %>
				 <a href= "getpagebooks?page=<%=index%>&code=<%=code%>&st=<%= st%>">[ <%=index %> ]</a> 
		        <%
				  index ++;
			 }
			 
			 if( index <= totalPage){%>
				  <a href="getpagebooks?page=<%=index%>&code=<%=code%>&st=<%= st%>">[다음>]</a>  
			<% } %>
        </div>
    </main>
